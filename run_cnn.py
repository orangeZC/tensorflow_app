...
train_dir = os.path.join('ex_train.txt')
test_dir = os.path.join('ex_test.txt')
val_dir = os.path.join('ex_val.txt')
vocab_dir = os.path.join('ex_vocab.txt')
...
def predict():
    ...
    predict_list = os.listdir('experimentdataset')

    for predict_file in predict_list:
        filename = 'experimentdataset/%s' % predict_file
        x_predict = predict_process_file(filename, word_to_id, config.seq_length)
        ...
        for i in range(num_batch):  # 逐批次处理
            ...
            y_pred_cls[start_id:end_id] = session.run(model.y_pred_cls, feed_dict=feed_dict)
        print('the class of %s: class%s' % (predict_file, str(y_pred_cls[0])))
    time_dif = get_time_dif(start_time)
    print("Time usage:", time_dif)
